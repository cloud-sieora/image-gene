...
const User = db.model('Dealer_creation', {
    ...
    access_type: { type: Number, default: 0 },
...
async function user_validate(name, pass, user_type) {
    const user = await User.findOne({ mail: name, password: pass, Active: 1, user_type: user_type })
    console.log(user)
    if (user == null) {
...
async function get_dealer_id(id) {
    const product = await User.findOne({dealer_id: id})
    return product
}
