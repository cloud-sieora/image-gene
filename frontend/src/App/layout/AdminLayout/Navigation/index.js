...
import Endusernavigation from '../../../../Enduser-menu-items';
...
class Navigation extends Component {
    ...
    render() {
        let navClass = [
            localStorage.getItem("cashier_big") == "1" ? 'pcoded-navbar2' : 'pcoded-navbar',
        ];
        ...
        let navContent = (
            <div >
                ...
                {/* <h5  style={{ fontSize: 17, fontFamily:'Poppins-SemiBold', marginLeft:10 ,color:'white',bottom:10}}>{localStorage.getItem("operator_id").toUpperCase()}</h5> */}

                <NavContent navigation={
                    localStorage.getItem('&s47$sfblm#5dfn') == "347fbxsdf*&^h&sdfkhj98sf*sdh0kjskdh*BDgdd&^%sdfg78&sdf876876adn" ?
                        Endusernavigation.items : null
                }
                />
            </div>
        );
        ...
        return (
            <Aux>
                <nav className={navClass.join(' ')} style={{ backgroundColor: '#181828' }}>
                    {navContent}
                </nav>
            </Aux>
        );
