...
import PlaybackCom from './PlaybackCom'
import PlayBack from '../PlayVideo/PlayBack';
...
export default function Events() {
  ...
  const [download_model, setdownload_model] = React.useState(false);
  ...
  const downloadClose = () => setdownload_model(false)
  const [download_camera_data, setdownload_camera_data] = useState([])
  const [download_camera_box, setdownload_camera_box] = useState(false)
  ...
  async function getimageurifunction1(accessKeyId, secretAccessKey, Bucket, data, i) {
    ...
    if (db_type == 'local') {
      const url = data.image_uri.split('localhost');
      image_uri = `${url[0]}10.147.20.214${url[1]}`
    } else {
    ...
    <>
      {
        userData.position_type != 'Attendance User' ?
          <div style={{ width: '100%' }} onClick={(event) => {
            ...
            <div>
              ...
              <Modal
                ...
                style={{ marginLeft: 'auto', marginRight: 'auto', top: '10%', width: '50%', }}
              >
                      ...
                      <div>
                        <div style={{ position: 'relative' }}>
                          <button className='eventbtn' id='cameras' onClick={() => {
                            setdownload_camera_box(!download_camera_box)


                          }} style={{ display: 'flex', backgroundColor: download_camera_box ? '#e22747' : '#e6e8eb', color: download_camera_box ? 'white' : 'black' }}> <AccessTimeIcon style={{ marginRight: '10px' }} />Cameras </button>

                          <div id='status' style={{ borderRadius: '5px', backgroundColor: '#181828', padding: '10px', position: 'absolute', top: '60px', zIndex: 1, boxShadow: 'rgba(0, 0, 0, 0.35) 0px 5px 15px', display: download_camera_box ? 'block' : 'none' }}>
                            <div style={{ position: 'relative' }}>
...

                                {
                                  data.length !== 0 ?
                                    <p style={{ margin: 0, color: 'white', cursor: 'pointer' }} onClick={() => {
                                      ...
                                      let camera = document.getElementsByClassName('downloadcameraCheckbox')
                                      for (let i = 0; i < camera.length; i++) {
                                        camera[i].checked = false
                                      }

                                      setdownload_camera_data([])
                              ...
                              <div className='lower_alerts' style={{ maxHeight: '200px', overflowY: 'scroll' }}>
                                {
                                  selectedcameras.length !== 0 ?
                                    selectedcameras.map((val, k) => {
                                      let typ = false
                                      download_camera_data.map((id) => {
                                        if (id == val._id) {
                                          typ = true
                                        }
                                      })
                                        ...
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px' }}>
                                          ...
                                          <input className='cameraCheckbox' checked={typ ? true : false} type="checkbox" onClick={(e) => {
                                            if (e.target.checked === true) {
                                              setdownload_camera_data([...download_camera_data, val._id])
                                            } else {
                                              let camcnk = []
                                              download_camera_data.map((cam) => {
                                                if (cam !== val._id) {
                                                  camcnk.push(cam)
                                                }
                                              })

                                              setdownload_camera_data(camcnk)
                                            }
                                          }}></input>
                                        </div>
                                      )
            ...
            <div style={{ height: '98vh', width: '100%', paddingRight: 0, }} >
              <Row id='main' >
                ...
                <Col xl={12} lg={12} md={12} sm={12} xs={12} id='events' style={{ display: page === 1 || window.screen.width > 990 ? 'block' : 'none', backgroundColor: '#e6e8eb', position: 'sticky', top: 0, zIndex: 2 }}>
                      ...
                      <div id='eventsDiv' style={{ display: 'flex', marginLeft: page === 1 ? '0px' : '10px', marginBottom: '10px' }}>
                        ...
                        <div className='eventsDiv1' style={{ position: 'relative' }}>
                          ...
                          {cameras.length != 0 ? (
                            ...
                            }}> <CalendarMonthIcon style={{ marginRight: '7px' }} />{`${startdate} (${starttime}) (${selected_hour})`} <ArrowDropDownIcon style={{ marginLeft: '10px' }} /></button>
                                ...
                                <div style={{ display: page === 1 ? 'none' : 'block' }}>
                                  <div style={{ display: 'flex', padding: '10px', flexDirection: 'column' }}>
                                    ...
                                    <div style={{ position: 'relative' }}>
                                      ...
                                      <div style={{ position: 'absolute', zIndex: 2, top: '50px', display: viewstart_date === true ? 'block' : 'none' }}>
                                        <DateComponent year={start_dateFullYear} month={Number(start_dateMonth)} date={Number(start_dateDate)} type={'start_date'} starttime={starttime} endtime={endtime} parentFunction={initialdate} flag={'hour'} select_hour={select_hour} apply_can_fun={apply_can_fun} days_time={true} />
                                      </div>
                                    </div>
                                  ...
                                  <div style={{ display: 'flex', justifyContent: 'flex-end', paddingRight: '10px' }}>
                                    <button className="my-2" style={{ backgroundColor: '#e32747', color: 'white', borderRadius: '5px', border: 'none', padding: '5px' }} onClick={() => {
                                      setclickbtn1(true)
                                      setapply_flag(true)
                                      document.getElementById('day').innerText = `${startdate} (${starttime})) (${selected_hour})`
                                      if (!second_screen) {
                                    ...
                                    <Col xl={12} lg={12} md={12} sm={12} xs={12} style={{ padding: '3px' }}>
                                      <div style={{ position: 'relative' }}>
                                        ...
                                        <div style={{ position: 'absolute', zIndex: 2, top: '90px', display: viewend_date === true ? 'block' : 'none' }}>
                                          <DateComponent year={end_dateFullYear} month={Number(end_dateMonth)} date={Number(end_dateDate)} type={'end_date'} starttime={starttime} endtime={endtime} parentFunction={initialdate} flag={'hour'} select_hour={select_hour} apply_can_fun={apply_can_fun} days_time={true} />
                                        </div>
                                      </div>
                                    </Col>
                                  </Row>
                        ...
                        <div className='eventsDiv1'>
                          {/* <div style={{ position: 'relative' }}>
                            ...
                            {cameras.length != 0 ? (
                              ...
                              <button className='eventbtn' id='cameras' onClick={() => {
                                setclickbtn3(true)
                                setdownload_model(true)
                                setdownload_camera_data(selectedcameras)



                              }} style={{ display: 'flex' }}> <DownloadingIcon style={{ marginRight: '7px' }} />Download </button>
                  ...
                  <Row>
                    <Col xl={12} lg={12} md={12} sm={12} xs={12}>
                      <PlaybackCom apply_flag={apply_flag} selected_hour={selected_hour} />
                      {/* <PlayBack apply_flag={apply_flag} selected_hour={selected_hour}  /> */}
                      {/* {
                    second_screen ?
                      <PlaybackCom />
                      :
                      <div>
                        <p>Select cameras and time frame</p>
                      </div>
                  } */}
                    </Col>
                  </Row>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
            <p>Attendance user Doesn't have permission to access Playback</p>
          </div>
      }
    </>
  )
