...
let date_mouse = true
export default function Events({ data1, res, aditional_info, type, alarm_status, all_alert_count, res1, analytics1, res_next_data, call_next_data, list_type }) {
            ...
            <div
                ...
                onScroll={(event) => {
                    ...
                    if (isBottomReached && scroll) {
                        ...
                        if (selectedanalytics.length !== 0 && selected_cameras.length !== 0) {
                            if (response_start_length.start_count + 60 <= response_start_length.total) {
                                response_start_length.start_count += 30;
                    ...
                    function next_data() {
                        ...
                        if (selected_cameras.length !== 0) {
                            const ids = selected_cameras.map(val => val._id);
                            ...
                            axios.request(config)
                                .then((response) => {
                                    console.log(response.data.data);
                                    const newData = [...first_data, ...response.data.data];
                                    setfirst_data(newData);
                                    colflag = true;
                                    setcolcount(colcount + 30);
