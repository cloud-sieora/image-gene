...
export default function History() {
    ...
    const [selected_tag, setselected_tag] = useState(['Block List'])
    ...
    const [showdata_flag, setshowdata_flag] = useState(false)
    ...
    useEffect(() => {
        let combain_data = { 'block_list': [], 'overtime': [], 'unknow': [] }
        let count = 0
        selected_sites.map((val) => {
            const getStocksData = {
                method: 'post',
                maxBodyLength: Infinity,
                url: `${api.VEHICLE_PARKING_AlERT}`,
                headers: {
                    'Content-Type': 'application/json'
                },
                data: {
            ...
            axios(getStocksData)
                .then(response => {
                    console.log(response.data);
                    count = count + 1
                    combain_data.block_list = [...combain_data.block_list, ...response.data.block_list]
                    combain_data.overtime = [...combain_data.overtime, ...response.data.overtime]
                    combain_data.unknow = [...combain_data.unknow, ...response.data.unknow]
                    if (count == selected_sites.length) {
                        ...
                        main_flag = true
                        document.getElementById('blocklist').checked = true
                        setoriginaldata(current_data)
                        setshowdata_flag(!showdata_flag)
    ...
    function showdata() {
        if (selected_tag.length == 1) {
            if (selected_tag[0] == 'Block List') {
                setdata(originaldata.block_list)
            } else if (selected_tag[0] == 'Overtime') {
                setdata(originaldata.overtime)
            } else if (selected_tag[0] == 'Unknow') {
                setdata(originaldata.unknow)
            }
        } else if (selected_tag.length == 0) {
            setdata([])
        } else {
            setdata([...originaldata.block_list, ...originaldata.overtime, ...originaldata.unknow])
        }
    }
            ...
            <div style={{ padding: '10px' }}>
                <div style={{ display: 'flex' }}>
                    ...
                    <div style={{ position: 'relative', marginLeft: '5px' }}>
                        ...
                        }} style={{ display: 'flex', backgroundColor: filter ? '#e32747' : '#e6e8eb', color: filter ? 'white' : 'black' }}> <CheckCircleOutlinedIcon style={{ marginRight: '10px' }} />Filter<ArrowDropDownIcon style={{ marginLeft: '10px' }} /></button>

                        <div>
                            <div style={{ borderRadius: '5px', backgroundColor: '#181828', padding: '10px', position: 'absolute', top: '60px', zIndex: 1, boxShadow: 'rgba(0, 0, 0, 0.35) 0px 5px 15px', width: '200px', display: filter ? 'block' : 'none' }}>
                                <div style={{ position: 'relative' }}>
                                    ...
                                    <div>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                                            <p style={{ margin: 0, color: 'white', fontWeight: 'bolder', marginBottom: '0px', marginRight: '10px' }}>Block List</p>
                                            <input type='checkbox' id='blocklist' onChange={(e) => {
                                                if (e.target.checked == true) {
                                                    setselected_tag([...selected_tag, 'Block List'])
                                                } else {
                                                    if (selected_tag.length == 1) {
                                                        setselected_tag([])
                                                    } else {
                                                        setselected_tag(['Overtime', 'Unknow'])
                                                    }
                                                }
                                                setshowdata_flag(!showdata_flag)
                                        ...
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                                            <p style={{ margin: 0, color: 'white', fontWeight: 'bolder', marginBottom: '0px', marginRight: '10px' }}>Overtime</p>
                                            <input type='checkbox' onChange={(e) => {
                                                if (e.target.checked == true) {
                                                    setselected_tag([...selected_tag, 'Overtime'])
                                                } else {
                                                    if (selected_tag.length == 1) {
                                                        setselected_tag([])
                                                    } else {
                                                        setselected_tag(['Block List', 'Unknow'])
                                                    }
                                                }
                                                setshowdata_flag(!showdata_flag)
                                        ...
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                                            <p style={{ margin: 0, color: 'white', fontWeight: 'bolder', marginBottom: '0px', marginRight: '10px' }}>Unknow</p>
                                            <input type='checkbox' onChange={(e) => {
                                                if (e.target.checked == true) {
                                                    setselected_tag([...selected_tag, 'Unknow'])
                                                } else {
                                                    if (selected_tag.length == 1) {
                                                        setselected_tag([])
                                                    } else {
                                                        setselected_tag(['Block List', 'Overtime'])
                                                    }
                                                }
                                                setshowdata_flag(!showdata_flag)
