...
export default function Index() {
    ...
    useEffect(() => {
        ...
        if (userData.position_type == 'Client' || userData.position_type == 'Client Admin') {
            ...
            axios(getStocksData)
                .then(response => {
                    console.log('get camera health first in dashboard', response.data);
            ...
            (JSON.parse(localStorage.getItem("userData"))).site_id.map((val) => {
                ...
                axios(getStocksData)
                    .then(response => {
                        ...
                        if (count == (JSON.parse(localStorage.getItem("userData"))).site_id.length) {
                            console.log('get camera health second in dashboard', data1)
            ...
            axios(getStocksData)
                .then(response => {
                    console.log('get camera health third in dashboard', response.data);
                    get_camera_health(response.data)
    ...
    function get_camera_health(cameras_res) {
        // console.log('camera res in dashboard', cameras_res)
        let data1 = []
        let count = 0
        let online = 0
        let offline = 0
            ...
            >
                <div style={{ backgroundColor: 'grey', borderRadius: '5px', overflowX: 'hidden' }}>
                    <Row style={{ padding: '0px', alignItems: 'center', }}>
                        <Col xl={12} lg={12} md={12} sm={12} xs={12}>
                            <div style={{ backgroundColor: '#e22747', color: 'white', display: 'flex', justifyContent: 'space-between', padding: '10px', alignItems: 'center' }}>
                                <p style={{ fontWeight: 'bold', padding: 0, margin: 0 }}>Add Canera(s)</p>
                                        ...
                                        skeleton == true ?
                                            (
                                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                                    <div>
                                                        <input type='text' placeholder='Search' style={{ backgroundColor: '#e6e8eb', color: 'black', padding: '10px', borderRadius: '20px', border: '1px solid gray', marginRight: '20px' }} value={top_camera_search} onChange={(e) => {
                                                            if (e.target.value !== '') {
                                                                ...
                                                                setcameras_view1(cameras_search)
                                                    ...
                                                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                                        <p style={{ color: 'black', fontSize: '20px', margin: 0, marginRight: '10px' }}>{selectedcameras.length} / {cameras_view.length} Selected</p>
                                                        <div style={{ position: 'relative' }}>
                                                                ...
                                                                </div>
                                                                {
                                                                    get_group_full_data_sort.length !== 0 ?
                                                                        <p style={{ margin: 0, color: 'white', cursor: 'pointer' }} onClick={() => {
                                                                            ...
                                                                            // groupelsefunction(cameras_view, 'regular')
                                                                        }}>Clear all</p>
                                                                        : ''
                                                                }
                                ...
                                <Col xl={12} lg={12} md={12} sm={12} xs={12} style={{ overflowX: 'scroll' }}>
                                    {
                                        skeleton ? (
                                            <table style={{ width: '100%', backgroundColor: 'white' }}>
                                                ...
                                                {console.log("Dashboard cameras view", cameras_view)}
                                                                    ...
                                                                    <div style={{ position: "relative", width: "150px", height: "100px" }}>
                                                                        <div style={{ position: "absolute", width: "100%", height: "100%", top: 0, left: 0, display: "flex", overflow: "hidden", justifyContent: "center", alignItems: "center" }}>
                                                                            ...
                                                                            <img style={{ display: 'none' }} id={`dash_image${i}`} width={150} height={100}
                                                                                src={''} alt='Tento vision image is loading' onError={(e) => {
