...
export default function PersonList() {
    ...
    const [displayLiveImage, setDisplayLiveImage] = useState(null)

    // const [dummyImage, setDummyImages] = useState(null)

    // const socket = io('https://tentovision1.cloudjiffy.net', {
    const socket = io('10.147.21.167:5008', {
        transports: ['websocket', 'polling'], // Ensure both WebSocket and polling are enabled
    });

    useEffect(() => {
        const handleCallSearchImage = async () => {
            try {
                await axios.get(api.GET_IMAGES_FOR_INITIAL).then(async (res) => {
                    ...
                    await axios.post('http://10.147.21.167:5008/read_image_from_path', res.data.data).then((res) => {
                        ...
                        if (res.data.data.length > 0) {
                            setDisplayLiveImage(res.data.data[0])
                        }
                    }).catch((err) => {
                        console.log('err', err)
                    })
        ...
        handleCallSearchImage()
    }, [])


    useEffect(() => {
        socket.on('search_image', async({ allImageData }) => {
            console.log('search all images form pandian', allImageData)
            await axios.post('http://10.147.21.167:5008/read_image_from_path', allImageData).then((res) => {
                // await axios.post('http://localhost:5008/read_image_from_path', res.data.data).then((res) => {
            ...
            // setFaceImagesFromFast(allImageData)
            // setDummyImages(allImageData[0])
        });

    }, []);

    console.log('--------------------------------', faceImagesFromFast)
            ...
            <div>
                ...
                <div style={{
                    ...
                    <div style={{
                        ...
                        {
                            selectedType === 'face' ?
                                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', height: '60px', width: '100px', backgroundColor: '#eaecee', borderRadius: '10px', padding: '5px 10px', justifyContent: 'space-evenly', marginLeft: '10px' }}>
                                    <div style={{ display: 'flex', marginRight: '5px' }}>
                                        ...
                                        />
                                        <p style={{ margin: 0, marginLeft: '5px', fontSize: '12px' }}>Face</p>
                                    </div>
                    ...
                    <div style={{
                    ...
                    }} >
                        {
                            displayLiveImage !== null ?
                                <img src={`data:image/jpeg;base64,${displayLiveImage.imageurl}`} alt="loading.." style={{
                                    height: '100%',
                                    width: '100%',
                                    objectFit: 'cover'
                                }} />
